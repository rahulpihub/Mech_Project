...
IMAGE_API_KEY = 'AIzaSyDKjWUM-ZChTcDYq3IIRnxV01aAyrfBFe0'
...
def analyze_image(image, api_key, data):
    try:
        ...
        data_str = ", ".join(data)
        prompt = f"""
        you are now an expert in mechanical image objects component detection,
...
# Function to scrape Amazon for product listings
def scrape_amazon(component):
    search_url = f"https://www.amazon.in/s?k={'+'.join(component.split())}"
    headers = {
    ...
    products = []
    for item in soup.find_all("div", {"class": "s-main-slot s-result-list s-search-results sg-row"}):
        for product in item.find_all("div", {"data-component-type": "s-search-result"}):
            title = product.h2.get_text(strip=True) if product.h2 else "No title"
            link = "https://www.amazon.in" + product.h2.a["href"] if product.h2.a else "No link"
            products.append((title, link))
            if len(products) >= 5:  # Limit to top 5 products
                break
        if len(products) >= 5:
            break
    return products
...
if uploaded_image is not None:
    ...
    if found_pdf and found_image is not None:
        ...
        # Ensure that 'defect' is initialized before this point
# For example, initialize it as an empty list if it's undefined
        # Ensure that 'defect' is initialized before this point
# For example, initialize it as an empty list if it's undefined
        defect = defect or []  # If defect is None, assign an empty list

        data_for_table = []  # Initialize the list to store table data

        if defect:  # Check if there are any defects
            for index, d in enumerate(defect, start=1):
                # Scrape for the first product related to the defect component
                products = scrape_amazon(d)

                if products:
                    title, link = products[0]  # Get the first product
                    data_for_table.append({
                        'S.no': index,
                        'Defect component': d,
                        'Amazon purchase link': f"[{title}]({link})"  # Use the product title as the link text
                    })
                else:
                    data_for_table.append({
                        'S.no': index,
                        'Defect component': d,
                        'Amazon purchase link': "No products found"
                    })

            # Display the data in a table format
            st.markdown("### Defected Components and Amazon Links")

            # Create a DataFrame for better formatting
            import pandas as pd
            df = pd.DataFrame(data_for_table)

            # Display the DataFrame using Streamlit's markdown for better formatting
            st.write(df.to_markdown(index=False))  # Display the DataFrame in a more readable format
        else:
